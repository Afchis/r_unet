...
class _BaseDataset(data.Dataset):
    ...
    def _get_labels(self, object):
        x = torch.tensor(object)
        for i in range(182):
            if i == 0:
                label = (x==i).float()
                depth = torch.tensor(morph.distance_transform_edt(np.asarray(label)))
    ...
    def __getitem__(self, index):
        ...
        # self._get_labels(label.astype(np.int64))
        labels, depths = self._get_labels(label)
        return torch.tensor(image), labels, depths
